...
ConstantValue* makeIntConstant(double i) {
...
ConstantValue* makeStringConstant(char* str) {
  ...
  value->strValue = strdup(str);
  return value;
}

ConstantValue* duplicateConstantValue(ConstantValue* v) { // ok
  ConstantValue* value = (ConstantValue*) malloc(sizeof(ConstantValue));
  value->type = v->type;
  if (v->type == TP_INT) 
    ...
    value->strValue = strdup(v->strValue);
    // strcpy(value->strValue, v->strValue);
  return value;
}
...
void freeObject(Object* obj) {
  switch (obj->kind) {
  case OBJ_CONSTANT:
    if(obj->constAttrs->value->type == TP_STRING)
      free(obj->constAttrs->value->strValue);
