...
Token* readIdentKeyword(void) {
  ...
  token->string = (char*)realloc(token->string, sizeof(char)*(MAX_IDENT_LEN+1));
  int count = 1;
...
Token* readNumber(int state) { // OK
  int ln=lineNo;
  int cn=colNo;
  char* number;
  number = malloc(sizeof(char)*100);
  int countNum=0; //dem so cac chu so
  int countDot=0;
  if(state==1){
    number[countNum++]='0';
    number[countNum++]='.';
  ...
  while(charCodes[currentChar]==CHAR_DIGIT || charCodes[currentChar]==CHAR_PERIOD){
    ...
    number[countNum]=currentChar;
    countNum++;
    readChar();
  }
  if(countDot>1){
    token = makeToken(TK_NONE, ln, cn);
    error(ERR_INVALID_DOUBLE, ln, cn);
    return token;
  }
...
Token* readConstChar(void) {
  ...
  token->string = (char*)realloc(token->string, sizeof(char)*2);
  readChar();
  if (currentChar == EOF) {
...
Token* getToken(void) {
  ...
  case CHAR_DIGIT: return readNumber(state);
  case CHAR_PLUS: 
